                    ...
                    with st.spinner("Memproses data time series..." if st.session_state.language == 'id' else "Processing time series data..."):
                        try:
                            ...
                            if st.session_state.data.empty:
                                st.error("Data kosong. Silakan upload data terlebih dahulu." if st.session_state.language == 'id' else "Data is empty. Please upload data first.")
                                st.stop()
                            
                            # Cek ketersediaan kolom
                            if date_column not in st.session_state.data.columns:
                                st.error(f"Kolom tanggal '{date_column}' tidak ditemukan dalam data." if st.session_state.language == 'id' else f"Date column '{date_column}' not found in data.")
                                st.stop()
                            
                            if target_column not in st.session_state.data.columns:
                                st.error(f"Kolom target '{target_column}' tidak ditemukan dalam data." if st.session_state.language == 'id' else f"Target column '{target_column}' not found in data.")
                                st.stop()
                            
                            # Cek jumlah data awal
                            original_length = len(st.session_state.data)
                            st.info(f"Memproses {original_length} baris data dengan frekuensi '{selected_freq}'..." if st.session_state.language == 'id' else f"Processing {original_length} data rows with frequency '{selected_freq}'...")
                            ...
                            st.success(f"Berhasil memproses {len(ts_data)} data points setelah resampling." if st.session_state.language == 'id' else f"Successfully processed {len(ts_data)} data points after resampling.")
                            
                        except ValueError as e:
                            error_msg = str(e)
                            if "Data terlalu sedikit" in error_msg or "Periode data terlalu pendek" in error_msg:
                                st.error(f"Error: {error_msg}")
                                st.info("Saran: Coba gunakan frekuensi yang lebih tinggi (misalnya 'D' untuk daily, 'M' untuk monthly) atau pastikan data mencakup periode yang lebih lama.")
                                st.stop()
                            else:
                                st.error(f"Error saat memproses data: {error_msg}" if st.session_state.language == 'id' else f"Error processing data: {error_msg}")
                                st.stop()
                        except Exception as e:
                            st.error(f"Error tidak terduga: {str(e)}" if st.session_state.language == 'id' else f"Unexpected error: {str(e)}")
                            st.stop()
