...
def test_forecasting_utils():
    ...
    try:
        result = train_arima_model(df, 'value', 'date', periods=30)
        print(f"   ✓ ARIMA model trained successfully")
        print(f"   - Original data points: {len(df)}")
        print(f"   - Missing values in target: {df['value'].isnull().sum()}")
    ...
    try:
        result = train_sarima_model(df, 'value', 'date', periods=30)
    ...
    try:
        result = train_sarimax_model(df, 'value', ['exogenous1', 'exogenous2'], 'date', periods=30)
    ...
    try:
        result = train_lstm_model(df, 'value', 'date', periods=30)
    ...
    try:
        result = train_exponential_smoothing(df, 'value', 'date', periods=30)
...
def test_feature_creation():
    ...
    try:
        df_with_lags = create_lag_features(df, 'value', lags=[1, 3, 7], fill_method='auto')
        print(f"   ✓ Lag features created successfully")
        print(f"   - New shape: {df_with_lags.shape}")
        print(f"   - Missing values after lag creation: {df_with_lags['value_lag_1'].isnull().sum()}")
        print(f"   - Missing values in lag_7: {df_with_lags['value_lag_7'].isnull().sum()}")
    except Exception as e:
    ...
    try:
        df_with_rolling = create_rolling_features(
            df, 'value', windows=[3, 7], 
            metrics=['mean', 'std', 'min', 'max'], 
            fill_method='auto'
        )
        print(f"   ✓ Rolling features created successfully")
        print(f"   - New shape: {df_with_rolling.shape}")
        print(f"   - Missing values in rolling_mean_3: {df_with_rolling['value_rolling_mean_3'].isnull().sum()}")
        print(f"   - Missing values in rolling_mean_7: {df_with_rolling['value_rolling_mean_7'].isnull().sum()}")
    except Exception as e:
    ...
    try:
        df_with_dates = create_features_from_date(df.reset_index(), 'date', 'value')
...
def test_anomaly_detection():
    ...
    try:
        results = detect_and_visualize_anomalies(
            df, 'value', 'date', methods=['ensemble'], 
            contamination=0.05, fill_method='auto'
        )
...
def test_data_validation():
    ...
    # Test validate_data_for_ml with different strategies
    strategies = ['auto', 'drop', 'impute']
    for strategy in strategies:
        try:
            print(f"\n1. Testing validation with strategy '{strategy}':")
            result = validate_data_for_ml(df_with_missing, 
                                         target_column='target', 
                                         handle_missing=strategy,
                                         verbose=False)
            
            if result['data'] is not None:
                print(f"   ✓ Validation successful with strategy '{strategy}'")
                print(f"   - Original shape: {df_with_missing.shape}")
                print(f"   - Cleaned shape: {result['data'].shape}")
                print(f"   - Missing handled: {result['missing_handled']}")
            else:
                print(f"   ✗ Validation failed: {result['errors']}")
                
        except Exception as e:
            print(f"   ✗ Failed with strategy '{strategy}': {str(e)}")
    
    # Test with very short time series
    print(f"\n3. Testing with very short time series:")
    try:
        short_df = df_with_missing.head(20).copy()
        result = train_arima_model(short_df, 'target')
        print(f"   ✓ Handled short series successfully")
    except Exception as e:
        print(f"   ✗ Failed with short series: {str(e)}")
...
def test_edge_cases():
    ...
    try:
        ...
        print(f"   - Original missing: {df['value'].isnull().sum()}")
    ...
    try:
        ...
        df.loc[100:300, 'value'] = np.nan
        
        result = validate_data_for_ml(df, target_column='value', missing_strategy='impute')
    ...
    try:
        ...
        short_df.loc[5:8, 'value'] = np.nan  # Add some missing
        
        result = train_arima_model(short_df, 'value', 'date', periods=5)
